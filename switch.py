...
class OreiMatrixOutputRoutingSelect(OreiMatrixOutputEntity, SelectEntity):
    ...
    Note: Entity IDs and attributes use numeric identifiers for automation stability.
    Custom names are cosmetic only - they appear in the UI but automations should
    use the services (orei_matrix.set_routing) with numeric input/output values,
    or reference the input_number/output_number attributes.
    """
    ...
    def __init__(
        ...
        self._optimistic_input_num: int | None = None

    @property
    def options(self) -> list[str]:
        """Return the list of available options (uses custom input names)."""
        return [self.coordinator.get_input_name(i) for i in range(1, NUM_INPUTS + 1)]

    @property
    def current_option(self) -> str | None:
        """Return the current selected option (custom name for UI display)."""
        if self._optimistic_option is not None:
            return self._optimistic_option
            
        if self.coordinator.data is None:
            return self.coordinator.get_input_name(self._output_num)
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        """Return stable numeric attributes for automations.
        
        Automations should use these numeric values which don't change when
        names are updated. Use orei_matrix.set_routing service with numbers.
        """
        if self._optimistic_input_num is not None:
            input_num = self._optimistic_input_num
        elif self.coordinator.data is not None:
            routing = self.coordinator.data.get("routing", {})
            input_num = routing.get(self._output_num, self._output_num)
        else:
            input_num = self._output_num
            
        return {
            "output_number": self._output_num,
            "input_number": input_num,
            # Also include name for reference (but automations should use numbers)
            "input_name": self.coordinator.get_input_name(input_num),
            "output_name": self.coordinator.get_output_name(self._output_num),
        }
    ...
    async def async_select_option(self, option: str) -> None:
        ...
        # Fallback: try parsing "Input X" format
        if input_num is None and option.lower().startswith("input "):
            try:
                input_num = int(option.lower().replace("input ", ""))
            except ValueError:
                pass
        
        if input_num is None:
            _LOGGER.error("Could not find input number for '%s'", option)
            self._optimistic_option = None
            self.async_write_ha_state()
            return
        
        self._optimistic_input_num = input_num
        self.async_write_ha_state()
        ...
        try:
            ...
            self._optimistic_option = None
            self._optimistic_input_num = None
            self.async_write_ha_state()
            raise
...
class OreiMatrixOutputScalerSelect(OreiMatrixOutputEntity, SelectEntity):
    ...
    def current_option(self) -> str | None:
        ...
        scaler_value = scaler_settings.get(self._output_num, "Bypass")
        
        if isinstance(scaler_value, str):
            scaler_lower = scaler_value.lower()
            for option in self._attr_options:
                if option.lower() == scaler_lower:
                    return option
