...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    # NOTE: Do NOT fetch port names here - we want entities to be created with
    # stable numeric names (Output 1, Input 1) first. Names are fetched AFTER
    # entity setup to ensure entity_ids are always based on numbers.
    ...
    # Set up platforms FIRST with default numeric names
    # This ensures entity_ids are always stable (e.g., output_1_source)
    await hass.config_entries.async_forward_entry_setups(entry, PLATFORMS)

    # NOW fetch port names from HTTP API - this will update friendly names
    # but entity_ids are already locked in
    try:
    ...
    # Start heartbeat only if poll interval is long (connection might go stale)
    # With short poll intervals (< 30s), polling itself keeps connection alive
    if scan_interval >= 30:
