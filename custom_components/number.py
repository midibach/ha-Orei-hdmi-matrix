...
class OreiMatrixCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    ...
    def __init__(
        ...
        # HTTP API status tracking
        self._http_api_status = "Unknown"
        self._last_name_sync: str | None = None
    ...
    @property
    def http_api_status(self) -> str:
        """Return HTTP API status."""
        return self._http_api_status

    @property
    def names_loaded(self) -> bool:
        """Return if names have been loaded from HTTP API."""
        return self._names_loaded

    @property
    def last_name_sync(self) -> str | None:
        """Return timestamp of last name sync."""
        return self._last_name_sync

    @property
    def name_sync_enabled(self) -> bool:
        """Return if name sync is enabled."""
        return self.config_entry.options.get(CONF_SYNC_NAMES, True)
    ...
    def start_heartbeat(self) -> None:
        ...
        if self._heartbeat_task is None or self._heartbeat_task.done():
            ...
            _LOGGER.debug("Started heartbeat task for %s", self.name)
    ...
    def stop_heartbeat(self) -> None:
        ...
        if self._heartbeat_task and not self._heartbeat_task.done():
            ...
            _LOGGER.debug("Stopped heartbeat task for %s", self.name)
        ...
        while True:
            try:
                ...
                if not self.api.is_connected:
                    _LOGGER.debug("Heartbeat: Not connected, attempting reconnection")
                    await self._attempt_reconnect()
                    continue
                
                # Send heartbeat
                healthy = await self.api.heartbeat()
                
                if healthy:
                    # Connection is good, reset reconnect delay
                    self._reconnect_delay = RECONNECT_MIN_DELAY
                    self._consecutive_failures = 0
                else:
                    # Heartbeat failed, attempt reconnection
                    _LOGGER.warning("Heartbeat failed, connection may be dead")
                ...
                _LOGGER.error("Error in heartbeat loop: %s", err)
                self._consecutive_failures += 1
                await asyncio.sleep(RECONNECT_MIN_DELAY)

    async def _attempt_reconnect(self) -> bool:
        """Attempt to reconnect with exponential backoff."""
        _LOGGER.info(
            "Attempting reconnection (delay: %ds, failures: %d)",
            self._reconnect_delay,
            self._consecutive_failures,
        )
        
        # Mark as unavailable during reconnection
        self._available = False
        
        # Wait with backoff before attempting
    ...
    async def async_fetch_names(self) -> bool:
        ...
        if not sync_names:
            self._http_api_status = "Disabled"
            self._names_loaded = False
            _LOGGER.debug("Name sync disabled, using defaults")
            return False

        # Reset status before attempting
        self._http_api_status = "Connecting..."
        ...
        try:
            ...
            # Check if we actually got valid names
            names_found = False
            ...
            if names_found:
                ...
                self._http_api_status = "Connected"
                
                # Update last sync time
                from datetime import datetime
                self._last_name_sync = datetime.now().isoformat()
                ...
                self._http_api_status = "No Names"
                _LOGGER.warning("HTTP API responded but no valid names found")
                return False
            ...
            if "timeout" in error_msg or "connect" in error_msg:
                self._http_api_status = "Timeout"
            elif "json" in error_msg:
                self._http_api_status = "Parse Error"
            else:
                self._http_api_status = "Error"
            
            _LOGGER.warning("Failed to fetch port names: %s (status: %s)", err, self._http_api_status)
            return False
