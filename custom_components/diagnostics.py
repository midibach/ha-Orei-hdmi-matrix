...
async def async_setup_entry(
    ...
    # HTTP API status sensor
    entities.append(OreiMatrixHttpApiStatusSensor(coordinator))
...
class OreiMatrixOutputSourceSensor(OreiMatrixOutputEntity, SensorEntity):
    ...
    The sensor state (native_value) shows the custom input name for UI display.
    For automations, use the stable numeric attributes:
    - input_number: The current input number (1-8), None if unknown
    - output_number: This output's number (1-8)
    - input_name: Custom name of current input (same as state)
    """
    ...
    @property
    def native_value(self) -> str | None:
        """Return the current source (uses custom input name if available)."""
        if self.coordinator.data is None:
            return None
    ...
    def extra_state_attributes(self) -> dict[str, Any] | None:
        ...
        return {
            ...
            "input_name": self.coordinator.get_input_name(input_num) if input_num else None,
        }


class OreiMatrixHttpApiStatusSensor(OreiMatrixEntity, SensorEntity):
    """HTTP API status sensor for Orei HDMI Matrix.
    
    Shows whether the HTTP API (used for port names) is working.
    States: Connected, Disconnected, Authentication Failed, Disabled
    """
    ...
    def __init__(self, coordinator: OreiMatrixCoordinator) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator, "http_api_status", "HTTP API Status")

    @property
    def native_value(self) -> str:
        """Return the HTTP API status."""
        return self.coordinator.http_api_status

    @property
    def extra_state_attributes(self) -> dict[str, Any] | None:
        """Return additional attributes."""
        return {
            "names_loaded": self.coordinator.names_loaded,
            "last_sync": self.coordinator.last_name_sync,
            "sync_enabled": self.coordinator.name_sync_enabled,
        }
